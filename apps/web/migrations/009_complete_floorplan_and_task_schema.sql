-- Complete migration to ensure both job_floorplans and generated_tasks tables exist
-- This migration ensures both tables are properly created with all required columns and constraints
...
-- Create generated_tasks table if it doesn't exist
...
DROP POLICY IF EXISTS "Users can view generated tasks for authenticated users" ON generated_tasks;
DROP POLICY IF EXISTS "Directors and ops can manage generated tasks" ON generated_tasks;
DROP POLICY IF EXISTS "Authenticated users can create generated tasks" ON generated_tasks;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'jobs') THEN
        -- Add foreign key for job_floorplans
        IF NOT EXISTS (
            SELECT 1 FROM pg_constraint 
            WHERE conname = 'fk_job_floorplans_job_id'
        ) THEN
            ALTER TABLE job_floorplans 
            ADD CONSTRAINT fk_job_floorplans_job_id 
            FOREIGN KEY (job_id) REFERENCES jobs(id) ON DELETE CASCADE;
        END IF;
        
        -- Add foreign key for generated_tasks
        IF NOT EXISTS (
            SELECT 1 FROM pg_constraint 
            WHERE conname = 'fk_generated_tasks_job_id'
        ) THEN
            ALTER TABLE generated_tasks 
            ADD CONSTRAINT fk_generated_tasks_job_id 
            FOREIGN KEY (job_id) REFERENCES jobs(id) ON DELETE CASCADE;
        END IF;
    END IF;
EXCEPTION
