  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [
        ...
        supabase.from("jobs").select("*", { count: "exact", head: true }).eq("status", "in_progress").is("deleted_at", null).then(r => r, () => ({ count: 0, error: null })),
        supabase.from("quotes").select("*", { count: "exact", head: true }).eq("status", "pending").is("deleted_at", null).then(r => r, () => ({ count: 0, error: null, data: [] })),
        supabase.from("vehicles").select("*", { count: "exact", head: true }).eq("in_use", true).is("deleted_at", null).then(r => r, () => ({ count: 0, error: null })),
        supabase.from("waste_loads").select("*", { count: "exact", head: true }).gte("booked_at", todayISO).is("deleted_at", null).then(r => r, () => ({ count: 0, error: null })),
        supabase.from("crew_usage").select("d, utilization").order("d", { ascending: false }).limit(1).then(r => r, () => ({ data: [{ utilization: 85 }], error: null })),
        supabase.from("buffer_usage").select("d, percent").order("d", { ascending: false }).limit(1).then(r => r, () => ({ data: [{ percent: 15 }], error: null })),
        supabase.from("finance_metrics").select("d, net_margin").order("d", { ascending: false }).limit(1).then(r => r, () => ({ data: [{ net_margin: 24800 }], error: null })),
        supabase.from("installs_by_day").select("*").order("d", { ascending: true }).limit(30).then(r => r, () => ({ data: generateMockInstallsData(), error: null })),
        supabase.from("activity_log").select("id, text, occurred_at").order("occurred_at", { ascending: false }).limit(12).then(r => r, () => ({ data: [], error: null })),
      ]);
