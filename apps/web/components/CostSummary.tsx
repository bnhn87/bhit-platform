...
export default function CostSummary({ jobId }: { jobId: string }) {
  ...
  useEffect(() => {
    ...
    })().catch((e: SupabaseError) => !cancel && setErr(e?.message || "Failed to load"));
    return () => { cancel = true; };
  ...
  if (err) return <div style={{ padding: 16, color: theme.colors.danger }}>{err}</div>;
  if (!row) return <div style={{ padding: 16 }}>No cost data.</div>;

  const n = (v: number | string) => Number(v || 0);
  const price = n(row.quoted_total);
  const labour = n(row.installer_days) * (rates.installer || 0);
...
function KPI({ title, value, highlight }: { title: string; value: string; highlight?: boolean }) {
  return (
    <div style={{ padding: 14 }}>
      ...
      <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: "0.2px", color: highlight ? theme.colors.accent : theme.colors.text }}>
        {value}
      </div>
    </div>
  );
