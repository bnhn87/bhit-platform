...
import { getUploadUrl, deleteAsset, getSignedUrl } from "../../lib/storage";
import { supabase } from "../../lib/supabaseClient";
import { theme } from "../../lib/theme";
import { SupabaseError as _SupabaseError } from "../../lib/types";
...
export default function DrawingsPanel({ jobId }: { jobId: string }) {
  ...
  async function onUpload(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const file of Array.from(files)) {
        const result = await getUploadUrl(jobId, file, "drawings");
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {msg && <div style={{ color: theme.colors.textSubtle, marginTop: 8 }}>{msg}</div>}

      {rows.length === 0 ? (
        <div style={{ color: theme.colors.textSubtle, marginTop: 10 }}>No drawings yet.</div>
      ) : (
...
function DrawingRow({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await getSignedUrl(d.path, 3600);
        if (active && result.success) setUrl(result.data?.url || '');
      } catch {
        // ignore
      }
    })();
      ...
      <div style={{ width: 40, height: 40, borderRadius: 6, overflow: "hidden", background: "#111823", display: "flex", alignItems: "center", justifyContent: "center" }}>
        {url ? (
          ...
          /\.(png|jpe?g|webp|svg)$/i.test(d.name) ? (
            ...
            <span style={{ fontSize: 11, color: theme.colors.textSubtle }}>FILE</span>
          )
        ) : (
          <span style={{ fontSize: 11, color: theme.colors.textSubtle }}>...</span>
        )}
      </div>
