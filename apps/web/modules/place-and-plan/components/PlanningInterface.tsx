...
export default function PlanningInterface({ project: initialProject, onBack, onUpdateProject, saveStatus, forceSave }: PlanningInterfaceProps) {
  ...
  return (
    <div className="flex h-full bg-[var(--bg)] text-[var(--text)] font-sans">
      {isDraggingFile && (
        <FileUploadOverlay
      ...
      {/* Left Panel */}
      <aside className="w-80 bg-[var(--panel)] p-4 flex flex-col space-y-4 overflow-y-auto shadow-[var(--shadow)] z-10 border-r border-[var(--border)] backdrop-blur-md">
        <div className="flex gap-2">
          <button onClick={onBack} className="flex-grow p-2 bg-[var(--panelAlt)] hover:bg-[var(--muted)]/20 rounded-lg text-sm flex items-center gap-2 transition-colors justify-center text-[var(--text)] border border-[var(--border)]">
            <ArrowLeft size={16} />
        ...
        <div className="space-y-2 border-t border-[var(--border)] py-4">
          <input type='file' className="hidden" ref={fileInputRefFloor} disabled={isProcessing} accept="image/*,application/pdf" />
          <button onClick={handleFloorPlanClick} disabled={isProcessing} className="w-full text-white font-bold py-2 px-4 rounded-lg inline-flex items-center justify-center transition-colors bg-blue-600 hover:bg-blue-700 disabled:bg-[var(--muted)] disabled:cursor-wait border border-blue-500/30">
        ...
        <div className="space-y-2">
          <div className="p-3 bg-[var(--panelAlt)] border border-[var(--border)] rounded-lg space-y-2">
            <h4 className="text-xs font-bold text-[var(--muted)] mb-1 px-1">ACTIONS</h4>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setIsExportModalOpen(true)} className="flex-1 text-white font-bold py-2 px-2 rounded-lg inline-flex items-center justify-center transition-colors bg-[var(--panel-2)] hover:bg-[var(--panelAlt)] border border-[var(--border)]" title="Export Project">
            ...
            <button
              ...
              className={`w-full font-bold py-2 px-4 rounded-lg inline-flex items-center justify-center transition-colors disabled:bg-[var(--muted)] disabled:cursor-not-allowed ${isMeasuringMode ? 'bg-cyan-600 text-white' : 'bg-[var(--panel)] border border-[var(--border)] text-[var(--text)] hover:bg-[var(--panel-2)]'}`}>
              <Ruler className="w-4 h-4 mr-2" />
              <span>Measure Distance</span>
            </button>

            <button onClick={toggleMarqueeSelectMode} className={`w-full font-bold py-2 px-4 rounded-lg inline-flex items-center justify-center transition-colors ${isMarqueeSelectMode ? 'bg-blue-600 text-white' : 'bg-[var(--panel)] border border-[var(--border)] text-[var(--text)] hover:bg-[var(--panel-2)]'}`}>
          ...
          <div className="flex-shrink-0 pt-4 border-t border-gray-700">
            ...
            <div className="space-y-1.5 text-sm">
              {furnitureSummary.map(item => (
                <div key={item.name} className="p-2 bg-[var(--panelAlt)] rounded-md flex items-center gap-2.5 border border-[var(--border)]">
                  <div className="w-3 h-3 rounded-sm flex-shrink-0" style={{ backgroundColor: item.color || '#4a5568' }}></div>
                  <span className="flex-grow truncate select-none text-[var(--text)]">{item.name}</span>
                  <span className="font-mono text-white bg-[var(--muted)] px-2 py-0.5 rounded-md text-xs">
                    {item.placed}
                    {item.unplaced > 0 && <span className="text-white/70 ml-1">({item.unplaced})</span>}
                  </span>
                </div>
              ))}
            </div>
      ...
      {/* Center Panel */}
      <main className="flex-1 bg-[var(--bg)] p-4 flex items-center justify-center overflow-hidden relative">
        <FloorPlanCanvas
      ...
      {/* Right Panel */}
      <aside className="w-80 bg-[var(--panel)] p-4 overflow-y-auto border-l border-[var(--border)] backdrop-blur-md">
        ...
        <div className="space-y-3">
          {aiMessages.map((msg, i) => (
            <div key={i} className={`p-3 rounded-lg flex items-start gap-3 text-sm animate-fade-in ${msg.type === 'warning' ? 'bg-yellow-900/50 text-yellow-300 border border-yellow-700/50' :
